...
outer_corners = sketch.vertices().sort_by_distance(slide_center)[-4:]
sketch = fillet(
    outer_corners,
...
mouse_ears = [
    Plane.XY
    * Pos(
        pos.X + (-mouse_ear_shift if pos.X < slide_center.X else mouse_ear_shift),
        pos.Y + (-mouse_ear_shift if pos.Y < slide_center.Y else mouse_ear_shift),
        0,
    )
    * Cylinder(
...
slide = slide - magnet_holes + mouse_ears
show_object(slide)
slide.export_stl("slide-quarter-quarter-half.stl")
